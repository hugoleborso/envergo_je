...
from local import bulkCartoCreator, cartoCreator, main
...
def stressTest():
    t0 = time.time()
    main([(334155, 6701650)] * 100, 5, 25, [50, 75, 100, 130, 160], 8, 0.05)
    t1 = time.time()
...
def compareCartos(carto1, carto2, stretch=(1, 1)):
    ...
    saveArrayToCarto(
        diff,
        "local/output/diff/diff_" + carName1 + "_" + carName2 + ".asc",
        getCartoInfo(carto1),
    )
    plotAltiCarto(
        "local/output/diff/diff_" + carName1 + "_" + carName2 + ".asc",
    ...
    plt.savefig("local/output/diff/diff_" + carName1 + "_" + carName2 + ".png", dpi=500)
