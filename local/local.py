...
from utils import bassin_versant, carto, cartoOneTile
...
def main(
    ...
    OriginLessInnerCirclePoints, OriginLessQuadrantsPoints, _ = carto.createQuadrants(
        cartoPrecision, innerRadius, radii, quadrantsNb
    )
        ...
        if cartoMachine.queryOnePoint(point) is not None:
            ...
            for q in range(quadrantsNb):
                ...
                for i, _ in enumerate(radii):
                    quadrants[q].append([])
                    quadrants[q][i] = cartoMachine.queryAltis(
